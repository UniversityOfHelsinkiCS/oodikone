...
const checkGradeTable = async (page, gradesTableContents) => {
    ...
    .then($el => {
      ...
      cy.cs('Grade distribution-data')
        ...
        .within(() => {
          ...
          expect(page.locator('tr')).toHaveCount(gradesTableContents.length)
        })
    })
}
const checkTableContents = async (page, contents) => {
  page.locator('table tbody').scrollIntoViewIfNeeded()
  cy.get('table tbody').within(() => {
    ...
    expect(page.locator('tr')).toHaveCount(contents.length)
  })
}
const toggleShowGrades = async page => {
  cy.cs('gradeToggle', { force: true }).click({ force: true })
}
const toggleSeparateBySemesters = async page => {
  cy.cs('separateToggle', { force: true }).click({ force: true })
}
const openStudentsTab = async page => {
  cy.cs('StudentsTab').click()
}
const openAttemptsTab = async page => {
  cy.cs('AttemptsTab').click()
}
const searchByCourseName = async (page, courseName) => {
  page.locator("input[placeholder='Search by course name']").type(courseName)
}
const searchByCourseCode = async (page, courseCode) => {
  page.locator("input[placeholder='Search by course code']").type(courseCode)
}
const clickNewQueryButton = async page => {
  cy.cs('NewQueryButton').click()
}
const selectFromYear = async (page, year) => {
  cy.cs('FromYearSelector').click()
  cy.cs(`FromYearSelectorOption${year}`).click()
}
const openSummaryTab = async page => {
  cy.cs('SummaryTab').click()
}
const clickAway = async page => {
  page.locator('body').click(0, 0)
}
const getCourseSearchHooksForSingleYear = async (page, code) => {
  cy.intercept({
...
test.describe('Basic user', () => {
  ...
  test('Search should work on consecutive searches', async ({ page }) => {
    ...
    getCourseSearchHooksForSingleYear(page)
    searchByCourseCode(page)
    ...
    page.locator('text=Show population').click()
  ...
  test.describe('Course mappings work', () => {
    test('Searching single course having substitution mappings shows course statistics', async ({ page }) => {
      ...
      cy.contains('58131')
      openStudentsTab(page)
      cy.contains('svg', 'Pass rate')
      openAttemptsTab(page)
      cy.contains('svg', 'Pass rate')
      clickNewQueryButton(page)
  ...
  test.describe('Course populations', () => {
    test('Can find course population', async ({ page }) => {
      cy.contains('Search for courses')
      searchByCourseCode(page)
      page.locator('text=tr', 'text=TKT20003').click()
      ...
      page.locator('text=Students (19)').click()
      cy.contains('394776')
      cy.contains('416369')
    })
    test('Population of course shows grades for each student', async ({ page }) => {
      ...
      cy.contains('Include substitutions, Open and normal')
      page.locator('text=Students (33)').click()
  ...
  test('Language distribution is correct', async ({ page }) => {
    ...
    cy.contains('Include substitutions, Open and normal')
    page.locator('text=Language distribution').click()
  ...
  test.describe('Single course stats', () => {
    test.describe('Combine substitutions off', () => {
      ...
      test('Time range', async ({ page }) => {
        ...
        expect(page.locator('[data-cy^="FromYearSelectorOption"]')).toHaveCount(8)
        clickAway(page)
        cy.cs('ToYearSelector').click()
        cy.cs('ToYearSelectorOption2023-2024').should('have.class', 'Mui-selected')
        expect(page.locator('[data-cy^="ToYearSelectorOption"]')).toHaveCount(8)
        clickAway(page)
        cy.contains('Show population').should('be.enabled')
      })
      test.describe.skip('Students tab', () => {
        ...
        test('Show grades off, Separate by semesters on', async ({ page }) => {
          ...
          toggleSeparateBySemesters(page)
          checkTableContents(page)
        })
      ...
      test.describe.skip('Attempts tab', () => {
        test.beforeEach(async ({ page }) => {
          openAttemptsTab(page)
        })
        test.describe('Info boxes', () => {
    ...
    test.describe('Combine substitutions on', () => {
      ...
      test('Time range', async ({ page }) => {
        ...
        expect(page.locator('[data-cy^="FromYearSelectorOption"]')).toHaveCount(25)
        clickAway(page)
        cy.cs('ToYearSelector').click()
        cy.cs('ToYearSelectorOption2023-2024').should('have.class', 'Mui-selected')
        expect(page.locator('[data-cy^="ToYearSelectorOption"]')).toHaveCount(25)
        clickAway(page)
        cy.contains('Show population').should('be.enabled')
      })
      test.describe.skip('Students tab', () => {
      ...
      test('After changing time range shows correct options', async ({ page }) => {
        ...
        clickAway(page)
        cy.cs('FromYearSelector').click()
        expect(page.locator('[data-cy^="FromYearSelectorOption"]')).toHaveCount(21)
        clickAway(page)
        cy.cs('ToYearSelector').click()
        expect(page.locator('[data-cy^="ToYearSelectorOption"]')).toHaveCount(8)
        clickAway(page)
    ...
    test('If no data available, provider organization(s) select is disabled', async ({ page }) => {
      searchByCourseCode(page)
      page.locator('text=TKT20014').click()
    ...
    test('Has right to see all the students, because course rovider is TKT', async ({ page }) => {
      page.goto('coursestatistics?courseCodes=%5B%22TKT10004%22%5D&separate=false')
      cy.cs('course-population-for-2021-2022').click()
      // cy.contains('Students (29)').click()
    })
  })
})
test.describe.skip('Only course statistics', () => {
  test.beforeEach(async ({ page }) => {
  ...
  test('Some features of Course Statistics are hidden for courseStatistics-users without other rights', async ({
    ...
    searchByCourseCode(page)
    cy.wait('@courses').its('response.statusCode').should('be.oneOf', [200, 304])
    page.locator('text=tr', coursecode).click()
    cy.wait('@yearlystats').its('response.statusCode').should('be.oneOf', [200, 304])
    cy.url().should('contain', `courseCodes=%5B%22${coursecode}%22%5D`)
