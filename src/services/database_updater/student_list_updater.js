...
async function run () {
  ...
  for (let i = minStudentNumber; i < maxStudentNumber; i++) {
    ...
    if (i % step === 1) {
      ...
      console.log(iteration, '/', range / step + 1, timestamp() )
    }
  ...
  if (cached === null) {
    ...
    console.log('ERROR in student lists. There were', cached.student_numbers.length, 'students, was', cached.description)
    console.log('found: valid', validStudents.length, 'out of', maxStudentNumber - minStudentNumber)
