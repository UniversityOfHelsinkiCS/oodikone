...
const MIN_YEAR = 1899
const MAX_YEAR = 2112
const MIN_SELECTABLE_SEMESTER = '1950-51'
...
const getCourseYears = course => ({
  ...
  endYear: moment(course.enddate).year()
})
...
const getCourseSemesters = (course) => {
  ...
  return {
    ...
    end: getYearText(endYear, isSpring(course.enddate))
  }
}
...
const getStartAndEndYearValues = (course, years) => {
  ...
  const startYear = Number(start.substring(0, 4)) < 1950
    ? years.find(year => year.text === MIN_SELECTABLE_SEMESTER)
