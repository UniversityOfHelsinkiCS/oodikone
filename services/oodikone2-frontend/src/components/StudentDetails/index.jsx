...
class StudentDetails extends Component {
  ...
  renderStudyRights = () => {
    ...
    const studyRightRows = student.studyrights.map(studyright => {
      const degrees = sortBy(studyright.studyrightElements, 'enddate')
        .filter(e => e.element_detail.type === 10)
        .map(degree => ({
                  ...
                  <Table.Row key={c.studyrightid}>
                    <Table.Cell verticalAlign="middle">
                      {c.elements.degrees.filter(filterDuplicates).map(degree => (
                        <p key={degree.name}>
                          {`${degree.name} (${reformatDate(degree.startdate, 'DD.MM.YYYY')} - ${reformatDate(
                            degree.enddate,
                            'DD.MM.YYYY'
                          )})`}{' '}
                          <br />{' '}
