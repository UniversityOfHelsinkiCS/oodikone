...
describe("Updater works", () => {
  ...
  describe("All metadata is written", () => {
    test("Organisations are written to database", async () => {
      ...
      expect(
        organizations
          .find(_ => _.code === "H50")
          .name.find(n => n.langcode === "fi").text
      ).toBe("Matemaattis-luonnontieteellinen tiedekunta");
    });
    ...
    test("Course realisation types are written to database", async () => {
      ...
      expect(
        courseRealisationTypes.map(_ => _.realisationtypecode)
      ).not.toContain(null);
    ...
    test("Semesters are written to database", async () => {
      ...
      expect(semesters.map(_ => _.semestercode)).toEqual(
        expect.arrayContaining([...Array(160).keys()].map(_ => ++_))
      );
    ...
    test("Credit types are written to database", async () => {
      ...
      expect(creditTypes.map(_ => _.credittypecode)).toEqual(
    ...
    test("Course types are written to database", async () => {
      ...
      expect(courseTypes.map(_ => _.coursetypecode)).toEqual(
        expect.arrayContaining([expect.any(Number)])
      );
    ...
    test("Disciplines are written to database", async () => {
      ...
      expect(disciplines.map(_ => _.discipline_id)).toEqual(
  ...
  describe("Updater writes students the right way", () => {
    test("Student info is written to database", async () => {
      ...
      expect(birthdate.toISOString()).toBe(
        new Date("1966-04-24 00:00:00+00").toISOString()
      );
      ...
      const {
        creditcount: creditcount1,
        dateofuniversityenrollment: dofue
      } = students[1];

      expect(creditcount1).toBe(380);
      expect(dofue.toISOString()).toBe(
        new Date("2003-07-31 21:00:00+00").toISOString()
      );
    });
    ...
    test("Courses are written to database", async () => {
      ...
      expect(latest_instance_date.toISOString()).toBe(
        new Date("2010-03-16 00:00:00+00").toISOString()
      );
      expect(is_study_module).toBeTruthy();
      expect(startdate.toISOString()).toBe(
        new Date("1899-12-31 00:00:00+00").toISOString()
      );
      expect(enddate.toISOString()).toBe(
        new Date("2112-12-20 00:00:00+00").toISOString()
      );
      expect(max_attainment_date.toISOString()).toBe(
        new Date("2010-03-16 00:00:00+00").toISOString()
      );
      expect(min_attainment_date.toISOString()).toBe(
        new Date("2010-03-16 00:00:00+00").toISOString()
      );
    ...
    test("Credits are written to database", async () => {
      ...
      expect(attainment_date.toISOString()).toBe(
        new Date("1997-10-21 00:00:00+00").toISOString()
      );
      ...
      expect(allCredits.map(_ => _.semestercode)).toEqual(
        expect.arrayContaining([expect.any(Number)])
      );
    });
    ...
    test("Semester enrollments are written to database", async () => {
      ...
      const {
        ...
        studentnumber,
        semestercode
      } = semesterEnrollments[0];

      expect(semesterEnrollments.length).toBe(28);
      expect(id).toEqual(expect.any(String));
      expect(enrollmenttype).toBe(1);
      expect(enrollment_date.toISOString()).toBe(
        new Date("2003-08-17 00:00:00+00").toISOString()
      );
    ...
    test("Study rights are written to database", async () => {
      ...
      expect(enddate.toISOString()).toBe(
        new Date("2010-04-12 00:00:00+00").toISOString()
      );
      expect(givendate.toISOString()).toBe(
        new Date("2004-07-31 00:00:00+00").toISOString()
      );
      ...
      expect(studystartdate.toISOString()).toBe(
        new Date("2004-07-31 00:00:00+00").toISOString()
      );
    ...
    test("element details are written to database", async () => {
      ...
      expect(name.en).toBe(
        "Government Decree (794/2004) on University Degrees"
      );
      expect(type).toBe(15);
      expect(createdAt).not.toBe(undefined);
      expect(updatedAt).not.toBe(undefined);
    ...
    test("study right elements are written to database", async () => {
      ...
      expect(id).toBe(11);
      expect(startdate.toISOString()).toBe(
        new Date("2003-07-31T21:00:00.000Z").toISOString()
      );
      expect(enddate.toISOString()).toBe(
        new Date("2007-08-24T21:00:00.000Z").toISOString()
      );
      expect(createdAt).not.toBe(undefined);
      expect(updatedAt).not.toBe(undefined);
    ...
    test("transfers are written to database", async () => {
      ...
      expect(id).toBe("1");
      expect(transferdate.toISOString()).toBe(
        new Date("2009-07-30 21:00:00+00").toISOString()
      );
      expect(createdAt).not.toBe(undefined);
      expect(updatedAt).not.toBe(undefined);
      expect(studentnumber).toBe("014272112");
  ...
  describe("Updater updates existing students correctly", () => {
    ...
    test("Updating credits works", async () => {
      const allCredits = await Credit.findAll({ order: [["id", "ASC"]] });
      const updatedCredit = allCredits.find(
        _ => _.id === students[0].studyAttainments[2].credit.id
      );
