...
const {
  ...
  getStudyAttainments
} = require('../src/services/doo_api_database_updater/oodi_interface')
...
const getStudents = (includedCourses, requiredCourses, timeframe) => async studentNumbers => {
  const responses = await Promise.all(studentNumbers.map(sn => getStudyAttainments(sn)))
  return responses.map((response, index) =>
    parseStudentInfo(includedCourses, requiredCourses, timeframe)(response, studentNumbers[index])
  )
}
...
const parseStudentInfo = (includedCourses, requiredCourses, timeframe) => (response, studentNumber) => {
  ...
  const defaCourses = studyAttainments
    .filter(studyAttainment =>
      Boolean(includedCourses.find(course => Boolean(studyAttainment.course_code.match(optionalAYprefixRegex(course)))))
    )
  ...
  const courseCompletions = includedCourses.map(course =>
    Boolean(
...
const checkRequiredCourses = requiredCourses => studyAttainments =>
  requiredCourses.reduce(
    (acc, course) =>
      acc &&
      Boolean(
        studyAttainments.find(studyAttainment =>
          Boolean(studyAttainment.course_code.match(optionalAYprefixRegex(course)))
        )
      ),
    true
  )
...
const toCsv = includedCourses => students => {
  ...
  const reportCsv = [
    ...
    ...students.map(
      student =>
        `"${student.student_number}",${student.credits},${Number(student.hasRequired)},${student.courseCompletions
  ...
  const creditTotalCsv = [creditTotalHeaders.join(','), creditTotals.join(',')].join('\n')

  return [reportCsv, creditTotalCsv]
}

const getStudentNumbers = async courseIds => {
  const responses = await Promise.all(courseIds.map(id => getCourseUnitRealisation(id)))
...
const createReport = async paramFileName => {
  ...
  if (!params.in.course_ids) {
    console.error(
      'No file was defined as input for DEFA course unit realisation codes. Set it as in.course_ids in the parameter file.'
    )
    process.exit(1)
  }
  const courseIds = String(fs.readFileSync(params.in.course_ids))
    .trim()
    .split('\n')
  ...
  if (!params.in.included_courses) {
    console.error(
      'No file was defined as input for included course codes. Set it as in.included_courses in the parameter file.'
    )
    process.exit(1)
  }
  const includedCourses = String(fs.readFileSync(params.in.included_courses))
    .trim()
    .split('\n')
  ...
  if (!params.in.required_courses) {
    console.error(
      'No file was defined as input for required course codes. Set it as in.required_courses in the parameter file.'
    )
    process.exit(1)
  }
  const requiredCourses = String(fs.readFileSync(params.in.required_courses))
    .trim()
    .split('\n')
