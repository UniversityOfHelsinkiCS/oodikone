...
const getCustomOpenUniCourses = async courseCodes => {
  ...
  const studentsWithCurrentStudyRight = allStudyrights
    .filter(
      right => moment(right.startdate).isSameOrBefore(moment()) && !moment(right.enddate).isSameOrBefore(moment())
    )
  ...
  for (const { studentnumber, email, secondary_email } of studentInfo) {
    ...
    // const studentStudyrights = allStudyrights.filter(studyRight => studyRight.studyrightStudentnumber === studentnumber)
