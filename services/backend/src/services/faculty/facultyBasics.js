...
const { getGraduatedStudyRights, getStartedStudyRights, studyrightsByRightStartYear } = require('./faculty')
const {
  checkCommissioned,
  checkTransfers,
  findRightProgramme,
...
const addProgramme = async (
  ...
  const { progId, name, code } = facultyProgrammes.filter(facultyProgramme => facultyProgramme.code === programme)[0]
  if (!(progId in stats)) {
    stats[progId] = {}
...
const getFacultyStarters = async (
  ...
  for (const code of programmes) {
    ...
    const studyRights = await getStartedStudyRights(faculty, code, since, studyRightWhere)
    let startedStudyRights = studyRights.filter(
      studyRight => !checkTransfers(studyRight, insideTransfers, insideTransfers)
    )
    if (!includeAllSpecials) {
      startedStudyRights = startedStudyRights.filter(
        studyRight =>
          !checkTransfers(studyRight, insideTransfersStudyRights, transfersToOrAwayStudyRights) &&
          !checkCommissioned(studyRight.studyRightElements)
      )
    }

    startedStudyRights.forEach(({ startDate, studyRightElements }) => {
      const { programme, programmeName } = findRightProgramme(studyRightElements, code)
      let programmeId = programme

      if (Object.keys(codes).includes(programme)) {
...
const getFacultyGraduates = async (
  ...
  const studyRightWhere = getExtentFilter(includeAllSpecials)
  for (const code of programmes) {
    let graduatedStudyRights = await getGraduatedStudyRights(faculty, code, since, studyRightWhere)
    if (!includeAllSpecials) {
      graduatedStudyRights = graduatedStudyRights.filter(
        studyRight =>
          !checkTransfers(studyRight, insideTransfersStudyRights, transfersToOrAwayStudyRights) &&
          !checkCommissioned(studyRight.studyRightElements)
      )
    ...
    graduatedStudyRights.forEach(({ endDate, extentCode, studyRightElements }) => {
      const { programme, programmeName } = findRightProgramme(studyRightElements, code)
      if (!studyRightElements.find(element => new Date(element.endDate).getTime() === new Date(endDate).getTime())) {
        return
      }
      let programmeId = programme
      if (Object.keys(codes).includes(programme)) {
      ...
      if (programmeFilter === 'ALL_PROGRAMMES' || isNewProgramme(programme)) {
        ...
        if (graduatedAsBachelor(extentCode, studyRightElements)) {
        ...
        } else if (graduatedAsMaster(extentCode, studyRightElements)) {
