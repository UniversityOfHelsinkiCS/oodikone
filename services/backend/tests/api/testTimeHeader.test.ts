...
describe('test time middleware', () => {
  it('uses the date from the x-test-now header', async () => {
    ...
    const response = await request(app).get('/').set('x-test-now', '2026-03-06T12:00:00.000Z')

    assert.strictEqual(response.text, '2026-03-06T12:00:00.000Z')
  })
  ...
  it('the acual current date if x-test-now header missing', async () => {
    ...
    assert.notStrictEqual(response.text, '2026-03-06T12:00:00.000Z')
  })
