...
void describe('Course yearly statistics', async () => {
  ...
  await it('should return correct amount of values for a course without substitutions (TKT10002)', async t => {
    const res = (await request(app)
    ...
    await it('should work correctly in specific cases (TKT10002)', async t => {
      const res = (await request(app)
        .get('/courseyearlystats?codes=TKT10002&combineSubstitutions=false')
        .set('shib-session-id', 'test')
        .set('uid', 'basic')
      ...
      await t.test('should include students who have failed a course and then completed it only once', () => {
        const stats2017 = body.unifyStats!.statistics.find(year => year.name === '2017-2018')!
      ...
      await t.test(
        "should mark student's enrollment time correctly with mismatched semestercode and enrollment_date_time",
        () => {
    ...
    await it('should work correctly in specific cases (MAT11002)', async t => {
      ...
      await t.test('should not include students for AY code', () => {
        const stats = body.unifyStats?.statistics!.find(year => year.name === '2017-2018')
        assert(stats && 'enrollments' in stats, 'Missing field enrollment in statsitics')
        assert(!('534980' in stats.attempts.categories.failed))
      })

      await t.test('should not include student with failed grade after passed grade', () => {
      ...
      await t.test('should include student with failed grade and passed AY grade', () => {
      ...
      await t.test('should include student with only approved grade', () => {
      ...
      await t.test('should calculate enrollments correctly', async tt => {
        await tt.test('- 2021-2022', () => {
        ...
        assert.strictEqual(
          body.unifyStats?.statistics.reduce((acc, yearStats) => acc + yearStats.attempts.categories.failed.length, 0),
          1
        )
        assert.strictEqual(
