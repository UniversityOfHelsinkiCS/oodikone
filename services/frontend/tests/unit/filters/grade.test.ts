...
const ARGS = { courseIds: ['TKT002'], from: '2024-01-01', to: '2024-12-31' }

void describe('gradeFilter', () => {
  void it('should group student numbers by their highest grade for the given courses', () => {
    const student = createStudent({
      courses: [createCourse({ course_id: 'TKT002', grade: '5' }), createCourse({ course_id: 'TKT002', grade: '3' })],
    })
  ...
  void it('should ignore courses outside the requested date range', () => {
    const student = createStudent({
      courses: [createCourse({ course_id: 'TKT002', grade: '5', date: new Date('2023-01-01') })],
    })
  ...
  void it('should exclude a student whose grade is not selected', () => {
    const student = createStudent({ courses: [createCourse({ course_id: 'TKT002', grade: '3' })] })
