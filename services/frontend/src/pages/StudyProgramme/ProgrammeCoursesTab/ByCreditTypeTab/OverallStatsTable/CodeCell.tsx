...
export const CodeCell = ({ code, groupId }: { code: string; groupId: string }) => {
  return (
    <Stack alignItems="center" direction="row" gap={1} justifyContent="space-between">
      <Typography variant="body2">{code}</Typography>
      <Link
        title={`Open course statistics for ${code}`}
        to={`/coursestatistics?courses=${encodeURIComponent(groupId)}&separate=false&substitutions=false`}
      >
