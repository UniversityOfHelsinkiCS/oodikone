...
export const StudyTracksAndClassStatisticsTab = ({
  ...
  const progressStats = calculateStats(studyTrackStats?.creditCounts, studyTrackStats?.graduatedCount, getTargetCreditsForProgramme(programmeCode))
  ...
  const progressComboStats =
    Object.keys(studyTrackStats?.creditCountsCombo ?? {}).length > 0
      ? calculateStats(studyTrackStats?.creditCountsCombo, studyTrackStats?.graduatedCount, getTargetCreditsForProgramme(programmeCode) + 180)
      : null
