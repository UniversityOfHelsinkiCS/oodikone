...
const tagsApi = RTKApi.injectEndpoints({
  endpoints: builder => ({
    ...
    createStudentTag: builder.mutation({
      query: ({ tag, studytrack, combinedProgramme }) => ({
        url: `/studenttags/${tag.studentnumber}`,
        method: 'POST',
        body: { tag, studytrack, combinedProgramme },
      }),
    }),
    createMultipleStudentTags: builder.mutation({
    ...
    deleteMultipleStudentTags: builder.mutation({
...
export const {
  ...
  useCreateStudentTagMutation,
  useCreateMultipleStudentTagsMutation,
  useDeleteMultipleStudentTagsMutation,
} = tagsApi
