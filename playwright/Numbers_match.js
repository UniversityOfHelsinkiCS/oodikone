...
const selectCourseStatus = async (page, courseCode, status) => {
...
test.describe('Numbers should match between', () => {
  test.describe('Programme courses, Course statistics and Course population (no substitutions, all years)', () => {
    ...
    test('in Course statistics', async ({ page }) => {
      courseData.forEach(([_courseOrModule, code, name, total, passed, _notCompleted, failed, enrolledNoGrade]) => {
    ...
    test('in Course population', async ({ page }) => {
      courseData.forEach(([_courseOrModule, code, name, total, passed, _notCompleted, failed, enrolledNoGrade]) => {
        ...
        cy.cs('courseFilter-popper').within(() => {
          page.locator('text=li', `${code} - ${name}`).click()
        })
        selectCourseStatus(page)
  ...
  test.describe('Programme courses, Course statistics and Course population (no substitutions, single years)', () => {
    ...
    test('in Programme courses', async ({ page }) => {
      ...
      page.locator('text=a', 'text=Matemaattisten tieteiden kandiohjelma').click()
      cy.cs('ProgrammeCoursesTab').click()
      yearlyData.forEach(([year, total, passed, _failed, _enrolledNoGrade, notCompleted]) => {
    ...
    test('in Course statistics', async ({ page }) => {
      yearlyData.forEach(([year, total, passed, failed, enrolledNoGrade, _notCompleted]) => {
        ...
        expect(page.locator('tbody tr')).toHaveCount(2)
    ...
    test('in Course population', async ({ page }) => {
      yearlyData.forEach(([year, total, passed, failed, enrolledNoGrade, _notCompleted]) => {
        ...
        cy.cs('courseFilter-selector').click()
        cy.cs('courseFilter-popper').within(() => {
          page.locator('text=li', 'text=MAT21003 - Vektorianalyysi I').click()
        })
        selectCourseStatus(page)
        cy.contains(`Students (${total})`)
        selectCourseStatus(page)
