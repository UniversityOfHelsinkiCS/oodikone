...
const fetchData = async (priority, msg) => {
  try {
    ...
    if (task.task === 'meta') {
      ...
      stan.publish('status', JSON.stringify({ task: task.task, status: 'FETCHED', priority }), err => {
        if (err) console.log('STATUS PUBLISH FAILED', err)
      })
      ...
      try {
        ...
        stan.publish('status', JSON.stringify({ task: task.task, status: 'FETCHED', active, priority }), err => {
          if (err) console.log('STATUS PUBLISH FAIELD', err)
        })
        stan.publish(
          priority ? 'PriorityWrite' : 'UpdateWrite',
          JSON.stringify({ task: task.task, data, active }),
          err => {
            if (err) console.log('STATUS PUBLISH FAILED', err)
          }
        )
        ...
        if (e.name === 'NO_STUDENT') {
          stan.publish(
            'status',
            JSON.stringify({ task: task.task, status: 'NO_STUDENT', active: false, priority }),
            err => {
              if (err) console.log('STATUS PUBLISH FAILED', err)
            }
          )
